...
function redirect($location = null)
{
	...
	<?php
			...
			?>
		<script>
			window.location = "<?php url() . print($location); ?>"
		</script>
	<?php
		}
	}
	...
	function encpt($string)
	{
		...
		print('<br>' . password_hash($string, PASSWORD_DEFAULT));
								...
								<tr class="text-secondary">
									...
									<td class="align-middle">
										<a href="<?php url(); ?>admnPnl/updatePost?post=<?php echo $data['pid']; ?>">
											<h4><?php echo ucwords($data['pname']); ?></h4>
										</a>
									</td>
									<td class="align-middle">
										<a href="<?php url(); ?>admnPnl/category">
											<h5 class="text-dark font-weight-bold"><?php echo strtoupper($data['cname']); ?></h5>
										</a>
									</td>
